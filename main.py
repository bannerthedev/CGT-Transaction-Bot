...
# ---------------- CONFIG (fill these) ----------------
TOKEN = "MTQ5NzA2OTYwOTg2MDIwNjYyMg.GvV3UT.JqOUh8-V6BwyJ9XVeH4roK061QU_S1zsqN3U4w"
GUILD_ID = 1495487938073460738  # your guild id as int

MATCH_TIMES_CHANNEL_ID = 1497075021267472435
ASSIGNMENTS_CHANNEL_ID = 1497075406472347690
TRANSACTIONS_CHANNEL_ID = 1496214947439444061
MATCH_SCORES_CHANNEL_ID = 1497075117807636540
# ----------------------------------------------------
...
async def code(interaction: discord.Interaction, team1: discord.Role, team2: discord.Role):
    ...
    # Generate random code: CGL + 4 digits
    code_value = f"CGL{random.randint(1000, 9999)}"
...
@bot.tree.command(
    guild=discord.Object(id=GUILD_ID),
    name="submit_time",
    description="Post a match time and create assignment panel (staff only)"
)
@discord.app_commands.describe(
    match_key="Short match identifier (e.g. Week3 TeamA vs TeamB)",
    time_str="Scheduled time (display text)",
    team1="Team 1 name",
    team2="Team 2 name"
)
async def submit_time(interaction: discord.Interaction, match_key: str, time_str: str, team1: str, team2: str):
    if not is_staff(interaction.user):
        await interaction.response.send_message("Only staff can use this.", ephemeral=True)
        return
    ...
    # Post to match times channel and assignments channel
    match_ch = bot.get_channel(MATCH_TIMES_CHANNEL_ID)
    assign_ch = bot.get_channel(ASSIGNMENTS_CHANNEL_ID)
    try:
        if match_ch:
            await match_ch.send(base_text)
        if assign_ch:
            # send with view for claiming if you want interactive AssignmentView
            await assign_ch.send(base_text)
    except Exception:
        logging.exception("Failed to post match time/assignment channels")

    # register assignment state
    assignments[match_key] = {"time": time_str, "ref": ref_text, "caster": caster_text}

    await interaction.response.send_message(f"Match time posted for {match_key}.", ephemeral=True)
# ---------- end submit_time ----------
...
async def captain_panel(interaction: discord.Interaction):
    ...
    embed = discord.Embed(
        title=f"CGL Captain Panel – {team_name}",
        description=desc,
    ...
    embed.set_footer(text="CGL Transactions Bot")
...
async def co_captain_panel(interaction: discord.Interaction):
    ...
    embed = discord.Embed(
        title=f"CGL Co-Captain Panel – {team_name}",
        description=desc,
...
@bot.tree.command(
    guild=discord.Object(id=GUILD_ID),
    name="info",
    description="Show information about the CGL Transactions Bot commands"
)
async def info(interaction: discord.Interaction):
    embed = discord.Embed(
        title="CGL Transactions Bot – Command Guide",
        description=(
            "What every command does and who is allowed to use it.\n\n"
            "**Bot Name:** CGL Transactions Bot\n"
            "**Created by:** banner"
        ),
    ...
    embed.add_field(
        ...
        value=(
            "Who can use it: **Staff**\n"
            "Create and post a CGL Season 6 bracket with chosen teams. "
            "Bracket auto‑updates as scores are reported."
        ),
        inline=False
    )

    embed.set_footer(text="CGL Transactions Bot • created by banner")
